...
export function Sidebar({ onClose, onSessionSelect, activeSessionId, collapsed, onToggleCollapse }: SidebarProps) {
  ...
  const { sessions, archivedSessions, showArchived, setShowArchived, loading: sessionsLoading, error: sessionsError, fetchSessions, fetchArchivedSessions, deleteSession, renameSession, togglePin, archiveSession, unarchiveSession } = useSessions();
  ...
  return (
    ...
    >
      ...
      {/* Chat History */}
      {!collapsed && (sessions.length > 0 || archivedSessions.length > 0) && (
        <div className="px-4 mb-1 flex items-center justify-between">
          <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/40">
            {showArchived ? `Archived (${archivedSessions.length})` : `Chats (${sessions.length})`}
          </span>
          <button
            onClick={() => {
              const next = !showArchived;
              setShowArchived(next);
              if (next) fetchArchivedSessions();
            }}
            ...
            {showArchived ? (
              ...
              archivedSessions.length === 0 ? (
                <div className="px-3 py-6 text-center space-y-2">
                  <Archive className="h-6 w-6 text-muted-foreground/20 mx-auto" />
                  <p className="text-xs text-muted-foreground/50 font-medium">No archived chats</p>
                  <p className="text-[10px] text-muted-foreground/30">Archived conversations will appear here</p>
                </div>
              ) : (
                archivedSessions.map((session) => (
                  <div
                    key={session.session_id}
                    className="group flex items-center gap-2 rounded-xl px-3 py-2 text-muted-foreground/50 hover:bg-muted hover:text-foreground transition-all duration-200"
                  >
                    <Archive className="h-3.5 w-3.5 shrink-0 opacity-50" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs truncate">{session.custom_name || session.auto_title || session.preview || "New conversation"}</p>
                      <p className="text-[10px] text-muted-foreground/30">{session.message_count} msgs &middot; {timeAgo(session.last_active)}</p>
                    </div>
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
                      <button
                        onClick={() => unarchiveSession(session.session_id)}
                      ...
                      <button
                        onClick={() => setDeletingSessionId(session.session_id)}
                        className="p-1 rounded-md hover:bg-red-400/10 hover:text-red-400 transition-colors"
