...
def test_session_persistence(mock_database):
    """Test that sessions persist to Supabase and can be read back."""
    ...
    with patch.object(mgr, "_make_conversation") as mock_make:
        ...
        # Verify data was written to mock DB
        sessions = mock_database.tables.get("sessions", [])
        assert len(sessions) >= 1
        assert any(s["user_id"] == "user1" and s["auto_title"] == "Test Chat" for s in sessions)
